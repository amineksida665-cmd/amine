...
-- العلامة المائية المحدثة لتظهر AMINE بدلاً من RIP، مع إمكانية فتح/إغلاق المود منيو بسرعة
local WatermarkBtn = Instance.new("TextButton", ScreenGui)
WatermarkBtn.Size = UDim2.new(0, 55, 0, 45)
WatermarkBtn.Position = UDim2.new(0, 20, 0.5, -22)
WatermarkBtn.BackgroundColor3 = Color3.fromRGB(15, 25, 40)
WatermarkBtn.Text = "AMINE"
WatermarkBtn.TextColor3 = Color3.fromRGB(0, 255, 128)
WatermarkBtn.TextSize, WatermarkBtn.Font = 12, Enum.Font.GothamBold
...
WatermarkStroke.Color = Color3.fromRGB(0, 255, 128)
...
ScrollingFrame.CanvasSize = UDim2.new(0, 0, 0, 1550)
...
local function createToggleFeature(name, parent, callback)
    ...
    toggleBtn.BackgroundColor3 = Color3.fromRGB(25, 40, 60)
    toggleBtn.Text = "OFF"
    ...
    tStroke.Color = Color3.fromRGB(0, 120, 255)
    ...
    local function trigger()
        ...
        toggleBtn.BackgroundColor3 = state and Color3.fromRGB(10, 40, 20) or Color3.fromRGB(25, 40, 60)
        tStroke.Color = state and Color3.fromRGB(0, 200, 100) or Color3.fromRGB(0, 120, 255)
        callback(state)
    end
...
createTextBoxOption("SET FLY SPEED", "80", function(txt)
...
createToggleFeature("HIDE NAME", ScrollingFrame, function(state)
...
createToggleFeature("INVISIBILITY", ScrollingFrame, function(state)
...
createToggleFeature("ESP PLAYERS", ScrollingFrame, function(state)
    _G.EspEnabled = state
    if not state then 
...
createToggleFeature("ADMIN ESP", ScrollingFrame, function(state)
    _G.AdminEspEnabled = state
    if not state then AdminEspContainer:ClearAllChildren() end
end)

createToggleFeature("AIMBOT", ScrollingFrame, function(state)
    _G.AimbotEnabled = state
end)

createToggleFeature("BYPASS WALLS", ScrollingFrame, function(state)
    _G.BypassWalls = state
end)

createToggleFeature("FLY", ScrollingFrame, function(state)
    _G.FlyEnabled = state
end)

createToggleFeature("HIGH JUMP", ScrollingFrame, function(state)
    _G.HighJumpEnabled = state
end)

-- خاصية Desync المعدلة والمصلحة للعمل بسلاسة
createToggleFeature("DESYNC SYSTEM", ScrollingFrame, function(state)
...
createButtonWithIcon("SPAWN CLONE (COPY)", ScrollingFrame, function()
    local char = LP.Character
    if char then
...
local function refreshLocationsList()
    ...
    for index, loc in ipairs(SavedLocations) do
        ...
        lBtn.Size = UDim2.new(0.40, 0, 1, 0)
        ...
        nameBox.Position = UDim2.new(0.42, 0, 0, 0)
        nameBox.BackgroundColor3 = Color3.fromRGB(25, 40, 60)
        nameBox.Text = tostring(loc.Name)
        nameBox.TextColor3 = Color3.fromRGB(255, 255, 255)
        nameBox.TextSize, nameBox.Font = 10, Enum.Font.GothamBold
        nameBox.ZIndex = 52
        Instance.new("UICorner", nameBox).CornerRadius = UDim.new(0, 6)

        local delBtn = Instance.new("TextButton", row)
        delBtn.Size = UDim2.new(0.20, 0, 1, 0)
...
createButtonWithIcon("INSTANT TP", ScrollingFrame, function()
    InstantTPPopup.Visible = not InstantTPPopup.Visible
end)

createButtonWithIcon("MY LOCATIONS", ScrollingFrame, function()
    LocationsPopup.Visible = not LocationsPopup.Visible
end)
...
createButtonWithIcon("YOUR GANG (Select Players)", ScrollingFrame, function()
    GangPopup.Visible = not GangPopup.Visible
end)
...
RunService.RenderStepped:Connect(function()
    ...
    if _G.EspEnabled then
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LP and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                ...
                local isAdmin = CustomAdmins[pNameLower] or CustomAdmins[displayNameLower]
                ...
                if not isAdmin then
                    ...
                    hl.FillColor = blueColor
                    hl.OutlineColor = blueColor
                    ...
                    if infoLabel and hum then
                        ...
                        infoLabel.TextColor3 = blueColor
                    end
    ...
    if _G.FlyEnabled and root then
        ...
        local gamepadConnected = UserInputService:GetConnectedGamepads()
        if #gamepadConnected > 0 then
            local thumbstickPos = UserInputService:GetGamepadState(Enum.UserInputType.Gamepad1)
            for _, state in pairs(thumbstickPos) do
                if state.KeyCode == Enum.KeyCode.Thumbstick1 then
                    local stickVector = state.Position
                    if stickVector.Magnitude > 0.1 then
                        moveDir = moveDir + (Camera.CFrame.RightVector * stickVector.X) + (Camera.CFrame.LookVector * stickVector.Y)
                    end
                end
            end
        end
